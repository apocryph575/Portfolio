...
url_site = "https://www.thuiswinkel.org/leden-zakelijk/?page="
url_companies = []
data = []
...
def parser():
    ...
    for company in url_companies:
        ...
        name_company = comp.find('div', class_='bg-white rounded shadow position-relative').find('div',class_='p-3 p-md-4').find('div', class_='media').find('div', class_='media-body').find("h1").get_text()
        name_company = name_company[41:]
        name_company = name_company[:-38]
        try:
            url_company = comp.find('div', class_='rounded shadow p-3 p-md-4').find('div', class_='row').find('div',class_='col-md-30').find('div', class_='media').find('div', class_='media-body').find("a").get("href")
        except:
            print(f"Url is not on {company}")
            url_company = " "
        try:
            tel_company = comp.find('div', class_='rounded shadow p-3 p-md-4').find('div', class_='row').find('div',class_='col-md-30').find_all('div', class_='media')[2].find("div", class_="media-body").find("a").get_text()
        except:
            print(f"Telephone is not on {company}")
            tel_company = " "
        try:
            KvK_company = comp.find('div', class_='rounded shadow p-3 p-md-4').find('div', class_='row').find('div',class_='col-md-30').find_all('div', class_='media')[3].find("div", class_="media-body").get_text()
